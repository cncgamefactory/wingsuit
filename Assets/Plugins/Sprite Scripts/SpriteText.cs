...
// 					lineStart = curNewLine + 1;
	...
	protected void Layout_Single_Line()
	{
		...
		if (displayString.Length == 1)
			width = spriteFont.GetWidth(displayString) * worldUnitsPerTexel;
		else
			width = spriteFont.GetWidth(displayString) * worldUnitsPerTexel * characterSpacing;
	...
	protected void Layout_Line(Vector3 startPos, string txt, int charIdx)
	{
		...
		if (txt.Length > 1)
		{
			dx = (GetWidth(txt) - (spriteFont.GetWidth(txt) * worldUnitsPerTexel)) / ( ((float)(txt.Length)) - (1f + characterSpacing * 0.5f) );
		}
...
/*/
		...
		{
			if (ch != null)
				startPos.x += ch.xAdvance * worldUnitsPerTexel * dxCoef + dx;
